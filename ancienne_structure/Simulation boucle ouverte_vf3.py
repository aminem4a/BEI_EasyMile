...
def plot_axle_torques_uniform_vs_opti(log_uniform: SimLog, log_opti: SimLog,
                                     label_uniform: str, label_opti: str,
                                     out_front_png: Path, out_rear_png: Path):
    """Nouveaux plots demandés:
    - Couple essieu AV (uniforme vs opti)
    - Couple essieu AR (uniforme vs opti)

    Notes:
    - On trace les couples *par essieu* (somme des 2 roues), donc directement
      log.c_front_total et log.c_rear_total.
    """

    # --- Front axle torque ---
    fig = plt.figure(figsize=(10, 4.5))
    plt.plot(log_uniform.t_s, log_uniform.c_front_total, label=label_uniform)
    plt.plot(log_opti.t_s, log_opti.c_front_total, label=label_opti)
    plt.title("Couple — Essieu avant (comparaison)")
    plt.xlabel("Temps (s)")
    plt.ylabel("Couple essieu avant (N·m)")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(out_front_png, dpi=200)
    ...
    # --- Rear axle torque ---
    fig = plt.figure(figsize=(10, 4.5))
    plt.plot(log_uniform.t_s, log_uniform.c_rear_total, label=label_uniform)
    plt.plot(log_opti.t_s, log_opti.c_rear_total, label=label_opti)
    plt.title("Couple — Essieu arrière (comparaison)")
    plt.xlabel("Temps (s)")
    plt.ylabel("Couple essieu arrière (N·m)")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(out_rear_png, dpi=200)
...
def main():
    ...
    # 7) Nouveaux plots demandés: couples essieux AV/AR séparés, sans opti vs avec opti
    out_torque_front = THIS_DIR / 'torque_essieu_avant_sans_vs_avec_opti.png'
    out_torque_rear = THIS_DIR / 'torque_essieu_arriere_sans_vs_avec_opti.png'
    plot_axle_torques_uniform_vs_opti(
        log_uniform, log_opti,
        label_uniform=uniform.name,
        label_opti=opti.name,
        out_front_png=out_torque_front,
        out_rear_png=out_torque_rear,
    )
    ...
    print(f"- {out_front}")
    print(f"- {out_rear}")
    print(f"- {out_torque_front}")
    print(f"- {out_torque_rear}")
